...
select municipio from ms where municipio = 'Campo Grande'; -- certo (Todos os CG)

select municipio, vitima from ms where vitima >= 3; -- certo (Cidades com mais de 3 vitimas)

select municipio, vitima from ms where vitima = (select max(vitima) from ms);  -- certo (Cidade com mais número de vitimas)
select municipio, vitima from ms where vitima = (select min(vitima) from ms);  -- certo (Cidade com menor número de vitimas)

select count(*) as Total_registros from ms;   -- certo (Conta todos os registros)
select sum(vitima) as Total_vitimas from ms;   -- certo (Conta todos as vitimas)

select avg(vitima) as Media_crimes from ms;  -- certo (Média de crimes)

select municipio from ms where municipio like 'C%'; -- certo (Cidades que começam com C)
